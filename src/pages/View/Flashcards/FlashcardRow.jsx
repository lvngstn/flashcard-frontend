...
function FlashcardRow({flashcard}) {
    const { setChecked } = useFlashcards();
    const [checked, setCheckedState] = useState(flashcard.checked);

    const handleCheckedChange = (checked) => {
        setCheckedState(checked);
        setChecked(flashcard.id, checked);
    };
        ...
        <div className="flex flex-row hover:bg-neutral-900 border-t-neutral-800 border-t-2 gap-2 pt-1 pb-3 text-neutral-100 justify-between items-top text-sm">
            <div className="flex flex-row w-full">
                <div className="flex flex-row gap-2 w-full items-top h-fit">
                    <input defaultChecked onChange={(e) => setChecked(flashcard.id, e.target.checked)} type="checkbox" className="checked:bg-primary-700 h-fit checked:border-primary-700 checked:fill-primary-700 checked:stroke-primary-700 rounded focus:ring-0 focus:ring-offset-0" />
